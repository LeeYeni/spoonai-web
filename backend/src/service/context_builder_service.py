...
class ContextBuilderService:
    def __init__(self, faiss_service: FAISSService):
        ...
        # 동시에 실행할 요청 수를 5개로 제한
        self.semaphore = asyncio.Semaphore(5)

    async def get_review(self, restaurant: str) -> Tuple[bool, str]:
        """
        ...
        async with self.semaphore:
            review = await self.naver_client.search_blog_reviews(restaurant)
            return False, review
    ...
    async def build_search_context(self, x: float, y: float):
        # 카카오 식당 리스트 가져오기
        kakao_results = await self.kakao_client.search_restaurants_concurrently(x, y)
